...
//int   triangulate_(const cv::Mat &RT, std::vector<cv::Point2f> &p1,std::vector<cv::Point2f> &p2, const cv::Mat &CameraMatrix, vector<cv::Point3f> &vP3D, vector<bool> &vbGood,float th2=4);
int   triangulate_(const cv::Mat &RT, std::vector<cv::KeyPoint> &kp1,std::vector<cv::KeyPoint> &kp2,  const cv::Mat &CameraMatrix, vector<cv::Point3f> &vP3D,  const vector<float> &scaleFactors,vector<bool> &vbGood,float maxChi2=5.998 );
...
double  reprj_error(const ImageParams &ip,const Frame &f1,const Frame &f2, const std::vector<cv::DMatch> &matches,const std::vector<cv::Point3f> &objPoints,std::vector<double> *err=0,const cv::Mat &t21=cv::Mat())throw(std::exception);
...
cv::Mat ARUCO_bestMarkerPose(const vector<aruco::Marker> &marker_views, const vector<se3> &frameposes_f2g, const ucoslam::ImageParams &cp,float markerSize);
